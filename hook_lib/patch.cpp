...
bool UI_VerifyString_Detour() {
	return true;
}

bool UI_VerifyString() {
	return true;
}

bool ActivisionClanTagAllowed() {
	return true;
}

utils::hook::detour strtablegetcolval;
const char* StringTable_GetColumnValueForRow_hook(StringTable* table, int row, int column) {
	const char* res = strtablegetcolval.stub<const char*>(table, row, column);
	if (strstr(table->name, "mp/statstable.csv")) {
		if (column == 41) {
			if (row == 32) return "11"; // amax
			if (row == 13) return "8";
			if (row == 15) return "9";
			if (row == 19) return "1";
			if (row == 38) return "5"; // 
			if (row == 43) return "6"; // iw8_sn_golf28
		}
	}
	return res;
}

__int64 LUI_CoD_LuaCall_MarkLocalized(uintptr_t luaVM)
{
	unsigned int v1;
	const char* v3;
	const char* v4;
...
void hooks()
{
	...
	utils::hook::jump(0x141BA1E80_g, UI_VerifyString_Detour);
	utils::hook::jump(0x1419F6D90_g, LUI_CoD_LuaCall_MarkLocalized);
	...
	strtablegetcolval.create(0x1413E2B40_g, StringTable_GetColumnValueForRow_hook);
...
void patchGame()
{
	...
	// CascView Patch Test
//	utils::hook::nop(0x1412AFB62_g, 0x11);
