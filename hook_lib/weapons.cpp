	...
	if (!inFrontend) {
		...
		for (int i = 0; i < 550; i++) {
			...
			if (strstr(weap->szInternalName, "iw8") && !strstr(weap->szInternalName, "watch") && !strstr(weap->szInternalName, "execution") && !strstr(weap->szInternalName, "plyr")) {
				...
				weaponDefJson[weap->szInternalName]["fireType"] = 0;
...
void SwapCamo(const char* camoName, const char* imageName) {
	GfxImage* img = DB_FindXAssetHeader(ASSET_TYPE_IMAGE, imageName, 0).image;
	Camo* camo = DB_FindXAssetHeader(ASSET_TYPE_CAMO, camoName, 0).camo;
	if (img && camo) {
		for (int i = 0; i < camo->textureDefCount; ++i) {
			camo->textureDefs[i].image = img;
		}
	}
}
...
void Load_MZSDef() {
	char path[MAX_PATH + 1];
	strcpy(path, Dvar_GetStringSafe("LOOQOTRNTN"));
	strcat(path, "\\players\\awepdef.json");
	SwapCamo("camo_01a", "attachment_wm_sm_papa90_receiver_v16_lod1_mat_camoblend"); // NOT TOUCH
	SwapCamo("camo_01b", "attachment_wm_sn_alpha50_receiver_v2_lod1_mat_camoblend");
	// SwapCamo("camo_01c", "attachment_wm_ar_scharlie_receiver_v6_lod1_mat_camoblend"); // WHITE BUT NOT THE BEST
	SwapCamo("camo_01c", "body_mp_western_domino_1_2_lod1_sheen");
	SwapCamo("camo_01d", "loot_camo_11a_c&loot_camo_11a_s~4217712572128437468");  // NOT TOUCH
	SwapCamo("camo_01e", "attachment_vm_griptape_02_mcharlie_v8_n&attachment_vm_griptape_02_mcharlie_v8_g~1109912445949244836"); // NOT TOUCH
	SwapCamo("camo_01f", ""); 
	bool inFrontend = *(bool*)(0x1459D1750_g);
	if (!inFrontend) {
		if (file_exists(path)) {
			...
			Com_SetErrorMessage("[DLL ERROR] awdepdef.JSON is not present in players directory.");
		}
	}
	else {
		Com_SetErrorMessage("Loading Camos was successfull. Use the command ingame to load weapons stats changes.");
	}
}
